...
export default function AdminDashboard() {
  ...
  // Handle Winner Approval via Backend API (PATCH)
  const handleApprovePayout = async (winnerId: string) => {
    ...
    if (res.ok) {
      // Refresh list to display updated status
      fetchWinners();
    }
  };
    ...
    <div className="bg-slate-900 min-h-screen text-slate-100 py-10 px-4">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="border-b border-slate-800 pb-4">
          <h1 className="text-3xl font-extrabold text-amber-400">Administrator Control Center</h1>
          <p className="text-slate-400 text-sm mt-1">Manage draws, user verification, and platform analytics[cite: 3].</p>
        </div>

        {/* Draw Execution Engine */}
        <div className="bg-slate-950 border border-slate-800 p-6 rounded-xl space-y-6">
          <h2 className="text-xl font-bold text-white">Monthly Prize Draw Engine</h2>
          
          <div className="flex flex-col md:flex-row gap-6">
            <div className="space-y-2 flex-1">
              <label className="text-sm font-semibold text-slate-400">Select Draw Mode</label>
              <select
                value={drawType}
          ...
          {simulationResult && (
            <div className="bg-slate-900 p-4 rounded-md border border-slate-800">
              <p className="text-xs uppercase font-semibold text-slate-500">Simulated Winning Numbers:</p>
              <div className="flex gap-4 mt-2">
                {simulationResult.map((n, i) => (
                  <span key={i} className="bg-amber-500/20 border border-amber-500 text-amber-400 text-xl font-extrabold px-4 py-2 rounded-lg">
                    {n}
                  </span>
                ))}
              </div>
          ...
          <div className="border border-slate-800 rounded-lg overflow-hidden">
            <table className="w-full text-left text-sm text-slate-400">
              ...
              <tbody>
                {loadingWinners ? (
                  <tr>
                    <td colSpan={5} className="p-4 text-center text-slate-500">Loading payout queue...</td>
                  </tr>
                ) : winners.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-4 text-center text-slate-500">No proof submissions queued.</td>
                  </tr>
                ) : (
                  winners.map((w) => (
                    <tr key={w.id} className="border-t border-slate-800">
                      <td className="p-3 text-white">{w.profiles?.email || 'vedant@digitalheroes.com'}</td>
                      <td className="p-3 capitalize">{w.match_category.replace('_', ' ')}</td>
                      <td className="p-3 text-emerald-400 font-bold">${w.prize_amount}</td>
                      <td className="p-3 text-amber-400 font-semibold">{w.proof_url}</td>
                      <td className="p-3">
                        <button
                          onClick={() => handleApprovePayout(w.id)}
                          disabled={w.status === 'paid' || updatingId === w.id}
                        ...
                        >
                          {updatingId === w.id ? 'Updating...' : w.status === 'paid' ? 'Marked as Paid' : 'Approve & Pay'}
                        </button>
                      </td>
                    </tr>
                  ))
